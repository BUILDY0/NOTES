...
function huanyuan(qian, zhong) {
    ...
    const qianLeft = qian.slice(1, 1 + rootIndex);
    const qianRight = qian.slice(1 + rootIndex, qian.length);
    const zhongLeft = zhong.slice(0, rootIndex);
    const zhongRight = zhong.slice(rootIndex + 1, zhong.length);
    root.left = huanyuan(qianLeft, zhongLeft);
    root.right = huanyuan(qianRight, zhongRight);
    return root;
}
...
const root = huanyuan(qian, zhong);
